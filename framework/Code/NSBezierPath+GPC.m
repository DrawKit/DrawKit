...
+ (void)setPathUnflatteningPolicy:(DKPathUnflatteningPolicy)sp
{
    [[NSUserDefaults standardUserDefaults] setInteger:sp
                                               forKey:kDKCurveFittingPolicyDefaultsKey];
}
...
- (NSBezierPath*)pathFromPath:(NSBezierPath*)otherPath usingBooleanOperation:(gpc_op)op
{
    ...
    if ([[self class] pathUnflatteningPolicy] == kDKPathUnflattenAlways)
        ...
        [self getPathMoveToCount:NULL
                     lineToCount:NULL
                    curveToCount:&cs
                  closePathCount:NULL];
        [otherPath getPathMoveToCount:NULL
                          lineToCount:NULL
                         curveToCount:&co
                       closePathCount:NULL];
    ...
    return [self pathFromPath:otherPath
        usingBooleanOperation:op
              unflattenResult:simplify];
}
...
- (NSBezierPath*)pathFromUnionWithPath:(NSBezierPath*)otherPath
{
    ...
    if (!NSIntersectsRect([self bounds], [otherPath bounds])) {
        ...
        if ([self intersectsPath:otherPath])
            return [self pathFromPath:otherPath
                usingBooleanOperation:GPC_UNION];
        else {
            [self appendBezierPath:otherPath];
            return self;
        }
...
- (NSBezierPath*)pathFromDifferenceWithPath:(NSBezierPath*)otherPath
{
    if (!NSIntersectsRect([self bounds], [otherPath bounds]))
        ...
        return [self pathFromPath:otherPath
            usingBooleanOperation:GPC_DIFF];
}
