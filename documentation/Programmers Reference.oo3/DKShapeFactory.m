...
NSString*	kGCSpeechBalloonType = @"kGCSpeechBalloonType";
NSString*	kGCSpeechBalloonCornerRadius = @"kGCSpeechBalloonCornerRadius";
...
+ (NSBezierPath*)	roundRectSpeechBalloonInRect:(NSRect) rect params:(int) sbParams cornerRadius:(float) cr
{
	...
	if (( sbParams & kGCSpeechBalloonEdgeMask ) == kGCSpeechBalloonLeftEdge )
	{
		mainRect.origin.x += rect.size.width / 4.0;
		mainRect.size.width -= rect.size.width / 4.0;
	}
	else if (( sbParams & kGCSpeechBalloonEdgeMask ) == kGCSpeechBalloonRightEdge )
	{
		mainRect.size.width -= rect.size.width / 4.0;
	}
	else if (( sbParams & kGCSpeechBalloonEdgeMask ) == kGCSpeechBalloonTopEdge )
	{
		mainRect.size.height -= rect.size.height / 4.0;
	}
	else if (( sbParams & kGCSpeechBalloonEdgeMask ) == kGCSpeechBalloonBottomEdge )
	{
	...
	if( cr >= (mainRect.size.height /2) )
		cr = truncf(mainRect.size.height /2) -1;
	
	if( cr >= (mainRect.size.width /2) )
		cr = truncf(mainRect.size.width /2) -1;
	...
	if ( sbParams == ( kGCSpeechBalloonBottomEdge | kGCSpeechBalloonPointsLeft ))
	{
	...
	else if ( sbParams == ( kGCSpeechBalloonBottomEdge | kGCSpeechBalloonPointsRight ))
	{
	...
	if (( sbParams & kGCSpeechBalloonEdgeMask ) == kGCSpeechBalloonRightEdge )
	{
		if ( sbParams & kGCSpeechBalloonPointsUp )
		{
			[path lineToPoint: NSMakePoint( NSMaxX( rect ), NSMinY( rect ))];
	...
	// spur on bottom:
	if (( sbParams & kGCSpeechBalloonEdgeMask ) == kGCSpeechBalloonTopEdge )
	{
		if ( sbParams & kGCSpeechBalloonPointsRight )
		{
			[path lineToPoint: NSMakePoint( NSMaxX( rect ), NSMaxY( rect ))];
...
{
	return [[self class] roundRectInRect:bounds andCornerRadius:[param floatValue]];
}
...
- (NSBezierPath*)	speechBalloonInRect:(NSRect) rect objParam:(id) param
{
	...
	// key = kGCSpeechBalloonType, value = type flags (NSNumber as integer)
	// key = kGCSpeechBalloonCornerRadius, value = radius (NSNumber as float)
	int		sbtype = kGCStandardSpeechBalloon;
	float	radius = 16.0;
	...
	if ([param isKindOfClass:[NSDictionary class]])
	{
		...
		sbtype = [[p objectForKey:kGCSpeechBalloonType] intValue];
		radius = [[p objectForKey:kGCSpeechBalloonCornerRadius] floatValue];
	}
