...
- (NSString*)		styleScript
{
	...
	if ([self colour])
	{
		[s appendString:[NSString stringWithFormat:@" colour:%@", [[self colour] styleScript]]];
	...
	if ([self gradient])
	{
		[s appendString:[NSString stringWithFormat:@" gradient:%@", [[self gradient] styleScript]]];
	}
...
- (NSSize)		extraSpaceNeeded
{
	...
	if ([self shadow] != nil && [self enabled])
	{
		...
		float br = [[self shadow] shadowBlurRadius];
