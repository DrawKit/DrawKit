...
/** @brief Returns a grey RGB colour.
 ...
 Uses the RGB colorspace, not the greyscale colorspace you get with NSColor's grey
 method. 
...
/** @brief Returns a grey RGB colour with alpha.

 Uses the RGB colorspace, not the greyscale colorspace you get with NSColor's grey
 method.
 @param grayscale \c 0 to \c 1.0
 @param alpha \c 0 to \c 1.0
...
/** @brief Returns a copy ofthe receiver but substituting the hue from the given colour.
 ...
 @return a colour with the hue of \c color but the receiver's saturation and brightness.
 */
...
/** @brief Returns a copy ofthe receiver but substituting the hue and saturation from the given colour.
 ...
 @return a colour with the hue, sat of \c color but the receiver's brightness.
 */
- (NSColor*)colorWithHueAndSaturationFrom:(NSColor*)color;

/** @brief Returns a colour by averaging the receiver with \c color in RGB space.
...
/** @brief Returns a colour by averaging the receiver with \c color in HSB space.
...
/** @brief Returns a colour by blending the receiver with \c color in rgb space.
...
/** @brief Returns a colour by blending the receiver with \c color in HSB space.
 @param color Blend with this colour.
 @param blends An array of four values, each 0..1, specifies how components from each colour are
 blended.
 @return Blend of the two colours.
...
/** @brief Returns a colour by interpolating between the receiver and a second colour.
 ...
 @param interpValue A value between 0 and 1.
 @return a colour that is intermediate between the receiver and <code>secondColor</code>, in RGB space.
 */
