...
/** @brief A category on NSBezierPath for creating various unusual shape paths, particularly for engineering use.
*/
@interface NSBezierPath (Shapes)

// chains and sprockets

/** @brief Returns the path of a standard roller chain link on a horizontal alignment with link centres of 1.0. Other variants are derived from this
 using transformations of this path.
*/
+ (NSBezierPath*)bezierPathWithStandardChainLink;

/** @brief Returns the path of a standard roller chain link linking \c a to <code>b</code>. The distance \c a-b also sets the dimensions of the link and of course
...
/** @brief Returns a path representing a roller chain sprocket having the pitch and number of teeeth specified. The sprocket is centred at the
...
/** @brief Path consists of zig-zags along the top and bottom edges with a 60° angle, optionally capped and with joining lines.
 */
...
/** @brief Produces the side-on view of a hex head or nut. The diameter is the across-flats dimension: the diameter of the circle inscribed
...
/** @brief The path follows the edges of <code>aRect</code>, consisting of four pairs of lines that intersect at the corners. \c length sets the
