...
/** @brief This subclass of DKDrawableShape implements a protocol for obtaining shapes dynamically from a shape provider.
 ...
 \c DKShapeFactory (instances) can be used as a shape provider.
 ...
 Archiving works because the shape provider must comply with NSCoding. \c DKShapeFactory is compliant, even though in fact
 it has no ivars. While the shared \c DKShapeFactory instance is often specified as a shape provider, private instances
 ...
 be added to \c DKShapeFactory rather than having to make lots of individual subclasses of DKDrawableShape for each one.
*/
